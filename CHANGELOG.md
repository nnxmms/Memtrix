...
- Email gained a **trusted-sender allowlist** for a tighter security boundary. Set `trusted_senders` (a list of addresses) on the Email page of the control panel and Memtrix will only ever see mail from those senders — every other message is filtered out at the tool level and never reaches the agent, the reasoning memory, or the reactive poller. The filter is enforced inside the email tool itself (in `EmailManager.check`), so both on-demand checks and the background "react to mail" poller honour it. Matching is done on the exact parsed From address (case-insensitive), never the display name, so a message that merely puts a trusted address in its display name can't slip through; the allowlist also narrows the IMAP search server-side as an optimisation while the exact in-process check remains the real boundary. Leave the list empty to keep the previous behaviour of allowing mail from all senders.

## 2.41.0

- Memtrix can now **react to incoming mail** instead of only checking it when asked. With email enabled, a new `react_to_mail` toggle (off by default) starts a lightweight background poller that watches the mailbox over IMAP every `poll_interval_seconds` (default 60s, clamped to a 15s minimum). The instant genuinely new mail arrives, it pings the main agent with a system notification — reusing the exact in-process mechanism that delivers finished background-worker results — so the agent reads the message, triages it, and only interrupts you when something actually needs your attention, staying quiet on newsletters and routine noise. The poller peeks without ever marking mail read, so the agent's own `email_check` still sees it as new, and it seeds a baseline of the existing unread backlog on startup so turning the feature on (or restarting) never triggers a flood of notifications for old or while-offline mail. The notification is delivered to your most recently active room. Configure it all on the **Email** page of the web control panel.

## 2.40.0

- Reasoning memory now learns about the **people, projects, and places** you mention and tracks **dated events** tied to them — not just facts about you. When you talk about someone ("my sister Jenna"), the background deriver records facts about them and, once enough signal accumulates, promotes them to a compact, auto-curated profile card under `people/<slug>.md` (deriver-owned, just like `USER.md`). When a known person or thing is named in a message, their card is injected as transient context so the agent already knows who you mean. Statements like "Jenna's birthday party is this Saturday" are resolved to a concrete date and stored as events: upcoming ones within a week surface proactively every turn under a **📅 Upcoming** block, and once an event passes you get a one-time **🔔 Just passed** nudge. Recurring annual events (birthdays) roll forward automatically, and stale non-recurring events are pruned. A new `memory_event` tool lets the agent log, list, or cancel events directly, `memory_profile` gained an optional `name` argument to fetch a specific person's card, and the Web Control Panel's memory admin gained **People** and **Events** tabs to browse, add, forget, and delete them. The whole subsystem is governed by an `entity_memory` off-switch plus `entity_card_max_chars`, `entity_promote_threshold`, `event_lookahead_days`, `event_followup_days`, and `event_retention_days` config keys, and every person can be individually forgotten for privacy.

## 2.39.0

- Reasoning memory now models only the user. The dual-peer design that maintained a separate self-card for the agent (`MEMORY.md`) has been removed entirely. In practice that card was almost wholly redundant with the static `SOUL.md` / `BEHAVIOR.md` / `AGENT.md` persona files, and reasoning over the assistant's own replies created a subtle echo chamber while burning tokens on every turn. Dropping it means the background deriver no longer runs a second derivation pass over assistant messages, and the system prompt no longer carries the ~1,500-character self-card injection — leaner prompts and less background work per message, with no loss of grounding on who the user is. The deriver, profile card (`USER.md`), memory tools (`memory_profile`, `memory_search`, `memory_context`, `memory_conclude`), web memory admin, and config are all now user-only; the `dual_peer` setting and `MEMORY.md` template are gone.
